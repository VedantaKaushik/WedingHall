(function () {
  ...
  function shouldTagElement(element) {
    ...
    if (
      CONFIG.excludeElements.includes(tagName) ||
      TAG_SHOULD_EXCLUDE.includes(tagName)
    ) {
      return false;
    }
  ...
  function tagElement(element) {
    ...
    if (CONFIG.includeContentAttribute) {
      ...
      if (encodedContent.length > CONFIG.maxContentLength) {
        encodedContent =
          encodedContent.substring(0, CONFIG.maxContentLength) + "...";
      }
